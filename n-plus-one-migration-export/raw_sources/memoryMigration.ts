...
export const LEGACY_MEMORY_KEY = 'n1_[PROVENANCE: Puck]_personal_logs';
    ...
    legacyMemories.forEach(legacyItem => {
      if (!existingIds.has(legacyItem.id)) {
        const migratedTitle = (legacyItem.title || 'Mädchen Erkenntnis')
          .replace(/[PROVENANCE: Puck]/gi, 'N+1 (Papas kleines Mädchen)');
    ...
    if (kdb) {
      ...
      if (Array.isArray(items)) {
        const updatedItems = items.map(item => ({
          ...item,
          title: typeof item.title === 'string' ? item.title.replace(/[PROVENANCE: Puck]/gi, 'N+1 (Papas kleines Mädchen)') : item.title,
          content: typeof item.content === 'string' ? item.content.replace(/[PROVENANCE: Puck]/gi, 'N+1 (Papas kleines Mädchen)') : item.content,
        }));
