...
| **FreeLLMRouterService & Keyless Route Failover** | `runtime-verifiziert` | Server `/api/freellm/v0.5.0/*` handles route resolution, simulated rate limits, ADE checks, and fallback logic with client persistence. | Tested live endpoint execution; verified ADE hash `ade_sha256_keller_88f7a2d`. |
...
| **System Bug Hunt & AST Auto-Patching** | `runtime-verifiziert` | Client `systemErrorBus` catches runtime errors, triggers `GlobalErrorObserverContext`, and posts to `/api/bughunt/autofix` for patch verification. | Live subscription and dispatch mechanism verified during linter and runtime checks. |
| **Self-Aware Toolchain 400 Engine** | `runtime-verifiziert` | 400 pre-cataloged tools in `src/data/toolchain400.ts` executable via `/api/toolchain/execute/:toolId`. | Catalog API and execution routes compiled and verified. |
