...
const GENERATE_FORECAST_DATA = () => {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    points.push({
      time: timeLabel,
      predictedRisk: Number((3.5 + generateDeterministicNumber(0, 2.5) + (i > 7 ? (i - 7) * 1.2 : 0)).toFixed(1)),
      memoryPressure: Number((32 + generateDeterministicNumber(0, 10) + i * 1.5).toFixed(1)),
      queueLatencyMs: Number((2.1 + generateDeterministicNumber(0, 1.2) + i * 0.2).toFixed(2)),
      mitigatedRisk: Number((2.0 + generateDeterministicNumber(0, 1.2)).toFixed(1))
    });
  }
  return points;
};
    ...
    const interval = setInterval(() => {
      setForecastData(prev => {
        ...
        for (let i = 0; i < nextPoints.length; i++) {
          ...
          nextPoints[i] = {
            ...nextPoints[i],
            predictedRisk: Number((Math.max(1.5, currentRisk + (generateDeterministicNumber(0, 0.8) - 0.4))).toFixed(1)),
            memoryPressure: Number((Math.max(20, currentMem + (generateDeterministicNumber(0, 2.0) - 1.0))).toFixed(1))
          };
        }
        return nextPoints;
      });
  ...
  const handleTriggerMitigation = async () => {
    ...
    setTimeout(() => {
      setForecastData(prev => prev.map(p => ({
        ...p,
        predictedRisk: Number((1.5 + generateDeterministicNumber(0, 1.0)).toFixed(1)),
        memoryPressure: Number((28 + generateDeterministicNumber(0, 5)).toFixed(1))
      })));
