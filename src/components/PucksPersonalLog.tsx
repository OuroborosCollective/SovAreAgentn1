...
export const PucksPersonalLog: React.FC = () => {
  const [logs, setLogs] = useState<PuckPersonalInsight[]>(() => {
    const saved = localStorage.getItem('n1_puck_personal_logs');
    if (saved) {
      try {
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-white tracking-tight">Puck's Personal Insight Log</h2>
            ...
            <p className="text-xs text-zinc-400 mt-1 max-w-xl">
              Puck's immutable memory vault & Audit viewer. Autonomously records learned logic connections and verifies runtime consistency with the axiomatic core.
            </p>
          </div>
        </div>
