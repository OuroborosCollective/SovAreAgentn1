...
export const SystemConsoleViewer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const generateLog = () => {
      const typeRand = generateDeterministicNumber(0, 1);
      ...
      const text = processes[Math.floor(generateDeterministicNumber(0, 1) * processes.length)] + (type === 'warn' ? ' (WARN: High variance)' : type === 'error' ? ' [FAILED]' : ' [OK]');
      ...
      setLogs(prev => {
        const newLogs = [...prev, { id: getDeterministicTimestampMs(), text, type, time: new Date().toLocaleTimeString() }];
        return newLogs.length > 50 ? newLogs.slice(newLogs.length - 50) : newLogs;
      });
    };

    const intervalId = setInterval(generateLog, 2500 + generateDeterministicNumber(0, 2000));
